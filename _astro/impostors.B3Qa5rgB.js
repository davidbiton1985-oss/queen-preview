import{T as A,S as F,L as b,C as z,a as B,D as G,b as _,P as L,I as O,c as u,M as E,V as w,Q as R,d as T,e as U,f as V}from"./stage.3FfRNqHS.js";import"./Home.astro_astro_type_script_index_0_lang.DokCm0p_.js";const W=16,$=4,N=4,I=448/320,P=.7685,q=.105;async function k(M,a,c,t,l,D){if(!a.length)return;const r=await new A().loadAsync(`${M}trees/tree_a${t?"_m":""}.webp`);r.colorSpace=F,r.anisotropy=t?2:8,r.generateMipmaps=!0,r.minFilter=b,r.wrapS=r.wrapT=z;const i=c.fog,s=new B({uniforms:{map:{value:r},uS:{value:l},uGain:{value:1.3},fogColor:{value:i?i.color:new _(0)},fogDensity:{value:i?i.density:0}},side:G,transparent:t,alphaToCoverage:!t,depthWrite:!t,premultipliedAlpha:!1,vertexShader:`
      ...
      void main() {
        ...
        vec3 wp = ip + right * position.x * sc * aVar.y * ${I.toFixed(4)} + vec3(0., 1., 0.) * (position.y - ${q.toFixed(3)}) * sc;
      ...
      void main() {
        ...
        col = mix(col, fogColor, 1.0 - exp(-fogDensity * fogDensity * vFogDepth * vFogDepth));`).replace(/float ff =[\s\S]*?ff \* 0\.0\);/,""),t&&(a=[...a].sort((e,o)=>e.z-o.z||e.x*l-o.x*l));const h=new L(1,1).translate(0,.5,0),v=new Float32Array(a.length*2);a.forEach((e,o)=>{const S=Math.sin(e.x*12.9898+e.z*78.233)*43758.5453,C=S-Math.floor(S);v[o*2]=Math.round(C*4)-2,v[o*2+1]=.92+C*7.3%1*.16}),h.setAttribute("aVar",new O(v,2));const n=new u(h,s,a.length),f=new E,p=new R,m=new w,g=new w;a.forEach((e,o)=>n.setMatrixAt(o,f.compose(m.set(e.x,e.y,e.z),p,g.setScalar(e.h/P)))),n.frustumCulled=!1,n.castShadow=!1,n.receiveShadow=!1,c.add(n);const d=new T({colorWrite:!1,depthWrite:!1}),x=new u(new U(1,1),d,a.length),y=new u(new V(1,1,1,5),d,a.length);a.forEach((e,o)=>{x.setMatrixAt(o,f.compose(m.set(e.x,e.y+e.h*.68,e.z),p,g.set(e.h*.55,e.h*.3,e.h*.55))),y.setMatrixAt(o,f.compose(m.set(e.x,e.y+e.h*.22,e.z),p,g.set(e.h*.035,e.h*.44,e.h*.035)))});for(const e of[x,y])e.castShadow=!0,e.frustumCulled=!1,e.renderOrder=-1,c.add(e);D()}export{k as loadTrees};
